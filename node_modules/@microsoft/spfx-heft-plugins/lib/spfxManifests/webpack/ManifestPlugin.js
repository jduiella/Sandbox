                    ...
                    if (chunk.hasRuntime()) {
                        ...
                        if (chunk[AsyncComponentPlugin_1.CHUNK_COMPONENT_MAP]) {
                            for (const [asyncChunk, externalComponents] of chunk[AsyncComponentPlugin_1.CHUNK_COMPONENT_MAP].entries()) {
                                ...
                                asyncChunks[asyncChunk.name || `Unnamed Chunk (#${chunk.id})`] = {
                                    asyncComponentDependencies: Array.from(dependenciesForAudit.values())
                                };
                            }
                        }
                        if (componentDependenciesAuditFile.bundles[chunk.name]) {
                            componentDependenciesAuditFile.bundles[chunk.name].asyncChunks = asyncChunks;
                        }
                        else {
                            compilation.errors.push(new Error(`Unable to find bundle entry for chunk ${chunk.name} in componentDependenciesAuditFile. This ` +
                                'is probably because there was an issue generating the loader configuration for this component.'));
                        }
                    }
                }
